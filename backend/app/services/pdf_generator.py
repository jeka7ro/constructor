...
def _compute_pdf_data(work_order, client, is_invoice=False):
        ...
        for vol in volumes:
            ...
            if surface > 0:
                if 'chape' in lbl or 'sapa' in lbl or 'apă' in lbl or lbl.isdigit():
                    ...
                    chape_items.append({"desc": f"{_t['chape_pose']}{chape_num_suffix} {min(thick, std_thick)} cm", "qty": surface, "price": float(prices.get('base', 12.5))})
                    if extra_thick > 0:
                        ...
                        chape_items.append({"desc": f"{_t['extra_thick']}{chape_num_suffix} ({extra_thick} cm)", "qty": surface, "price": extra_thick * extra_rate})
                    if vol.get('has_foil'):
                        foil_desc = f"{_t['foil']} (Chape {c_idx})" if has_mult_chape else _t['foil']
                        chape_items.append({"desc": foil_desc, "qty": surface, "price": float(prices.get('foil', 1.2))})
                    if vol.get('has_mesh'):
                        mesh_desc = f"{_t['mesh']} (Chape {c_idx})" if has_mult_chape else _t['mesh']
                        chape_items.append({"desc": mesh_desc, "qty": surface, "price": float(prices.get('mesh', 2.5))})
                    if vol.get('has_fiber') or vol.get('has_duramint'):
                        fiber_desc = f"{_t['fiber']} (Chape {c_idx})" if has_mult_chape else _t['fiber']
                        chape_items.append({"desc": fiber_desc, "qty": surface, "price": float(prices.get('fiber', 2.5 if surface <= 200 else 2.0))})
                    ...
                    isolation_items.append({"desc": f"Isolation PUR{pur_num_suffix} {pur_thick} cm", "qty": surface, "price": pur_base})
                    if vol.get('pur_aspiration'):
                        isolation_items.append({"desc": f"Aspiration{pur_num_suffix}", "qty": surface, "price": float(prices.get('pur_opt_aspiration', 2.00))})
                    if vol.get('pur_niveller'):
                        isolation_items.append({"desc": f"Nivellement{pur_num_suffix}", "qty": surface, "price": float(prices.get('pur_opt_niveller', 4.25))})
                    if vol.get('pur_poncage'):
                        isolation_items.append({"desc": f"Ponçage{pur_num_suffix}", "qty": surface, "price": float(prices.get('pur_opt_poncage', 1.50))})
                    if vol.get('pur_protection'):
                        isolation_items.append({"desc": f"Protection{pur_num_suffix}", "qty": surface, "price": float(prices.get('pur_opt_protection', 1.50))})
                    ...
                    if pur_discount_pct > 0:
                        ...
                        isolation_items.append({"desc": f"Remise PUR{pur_num_suffix} ({pur_discount_pct}%)", "qty": 1, "price": -(total_pur * pur_discount_pct / 100)})
                    ...
                    if prices.get('custom_eps_price_flat') not in (None, ""):
                        ...
                        for t in tiers:
                            if eps_vol <= float(t.get('max_m3', 99999)):
                                ...
                                break
                    isolation_items.append({"desc": f"Isolation EPS ({eps_vol:.2f} m3)", "qty": 1, "price": eps_price})
                    
                    eps_discount_pct = float(prices.get('eps_discount_pct', 0))
                    if eps_discount_pct > 0:
                        isolation_items.append({"desc": f"{_t['discount']} EPS ({eps_discount_pct}%)", "qty": 1, "price": -(eps_price * eps_discount_pct / 100)})
                else:
                    other_items.append({"desc": lbl or "Volume", "qty": surface, "price": float(work_order.estimated_price or 0) / (surface or 1)})

        # Forfait (Seuil de surface) - imediat după Chape
        total_chape_surf = sum(float(v.get('quantity', 0) or 0) for v in chape_vols)
        if total_chape_surf > 0:
            custom_thresh = prices.get('custom_threshold')
            threshold_charge = 0
            if custom_thresh not in (None, ""):
                threshold_charge = float(custom_thresh)
            else:
                thresholds = prices.get('surface_thresholds') or []
                for t in thresholds:
                    min_s = float(t.get('min_sqm', 0))
                    max_s = float(t.get('max_sqm', 999999))
                    if min_s <= total_chape_surf <= max_s:
                        threshold_charge = float(t.get('extra_charge', 0))
                        break
            if threshold_charge > 0:
                chape_items.append({"id": "threshold", "desc": "Forfait", "qty": 1, "price": threshold_charge})

        has_chape = len(chape_items) > 0
        has_iso = len(isolation_items) > 0
        show_chape_sub = has_chape and (has_iso or len(chape_vols) > 1)
        show_iso_sub = has_iso and (has_chape or (len(pur_vols) + len(eps_vols) > 1))

        chape_subtotal = sum(float(i.get('qty', 1)) * float(i.get('price', 0)) for i in chape_items)
        iso_subtotal = sum(float(i.get('qty', 1)) * float(i.get('price', 0)) for i in isolation_items)

        items = []
        if show_chape_sub:
            items.append({"isHeader": True, "headerLabel": "Chape"})
        items.extend(chape_items)
        if show_chape_sub:
            items.append({"isSubtotal": True, "subtotalLabel": "Sous-total Chape", "subtotalAmount": chape_subtotal})

        if show_iso_sub:
            items.append({"isHeader": True, "headerLabel": "Isolation"})
        items.extend(isolation_items)
        if show_iso_sub:
            items.append({"isSubtotal": True, "subtotalLabel": "Sous-total Isolation", "subtotalAmount": iso_subtotal})
    ...
    for item in items:
        if item.get("isHeader"):
            h_lbl = item.get("headerLabel") or item.get("desc") or ""
            table_rows += f"""
            <tr style="background-color: #0f172a;">
                <td colspan="4" style="font-weight: 800; font-size: 11px; text-transform: uppercase; letter-spacing: 1px; color: #F7CA31; padding: 6px 10px;">{h_lbl}</td>
            </tr>
            """
            continue
        if item.get("isSubtotal"):
            st_lbl = item.get("subtotalLabel") or "Sous-total"
            st_amt = float(item.get("subtotalAmount", 0))
            table_rows += f"""
            <tr style="background-color: #f8fafc; border-top: 1px dashed #cbd5e1; border-bottom: 1px dashed #cbd5e1;">
                <td colspan="3" style="font-weight: 700; text-align: right; text-transform: uppercase; font-size: 11px; color: #475569; padding: 6px 10px;">{st_lbl} :</td>
                <td style="font-weight: 800; text-align: right; font-size: 11px; color: #0f172a; padding: 6px 10px;">€{st_amt:.2f}</td>
            </tr>
            """
            continue
    ...
    if discount_amount == 0 and float(prices.get("discount_pct", 0)) > 0:
        ...
        chape_total = 0
        for item in items:
            if item.get("isHeader") or item.get("isSubtotal"):
                continue
