...
const updates = JSON.parse(fs.readFileSync(frPath, 'utf8')); // Wait, the whole frJson

// But I only want the specific updates to avoid overwriting existing German translations with French ones.
// I will just use the same `updates` object I had before.
const updatesObj = {
  "isoflex": {
...
const langs = ['en', 'de', 'nl', 'ru', 'ro'];

langs.forEach(lang => {
    const file = `frontend/src/i18n/${lang}.json`;
    if (fs.existsSync(file)) {
        let json = JSON.parse(fs.readFileSync(file, 'utf8'));
        json = deepMerge(json, updatesObj);
