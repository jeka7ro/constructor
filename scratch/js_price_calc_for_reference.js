    ...
    if (wo.volumes && wo.volumes.length > 0) {
        wo.volumes.forEach((vol) => {
            ...
            if (isChape) {
                ...
                // Base
                let baseRate = 0;
                if (surface > parseFloat(p.base_threshold || 200)) {
                    baseRate = getPrice(p.base_large, pricingSettings?.base_price_sqm_large, 12.0);
                } else {
                    baseRate = getPrice(p.base, pricingSettings?.base_price_sqm, 12.5);
                }
