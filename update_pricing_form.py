            ...
            <div className="px-5 pt-1 pb-4">
                {activeTab === 'sapa' && (
                    <>
                        ...
                        <PriceRow label={t('pricing_settings.foil', 'Film Plastique (Foil)')} value={settings.plastic_foil_price_sqm} onChange={v => onSettingChange('plastic_foil_price_sqm', v)} />
                        <PriceRow label={t('pricing_settings.mesh', 'Treillis Métallique')} value={settings.metal_mesh_price_sqm} onChange={v => onSettingChange('metal_mesh_price_sqm', v)} />

                        <SectionHeader label={t('pricing_settings.section_fiber', 'Fibre / Duramint')} />
                        <PriceRow label={t('pricing_settings.fiber_small', 'Fibre / Duramint (petite surf.)')} sublabel={`≤ ${settings.fiber_large_threshold_sqm} m²`} value={settings.fiber_price_sqm} onChange={v => onSettingChange('fiber_price_sqm', v)} />
                ...
                {activeTab === 'pur' && (
                    <>
                        <SectionHeader label="Grille Base & Épaisseur" />
                        <PriceRow label="Prix de base (3cm, ≤100m²)" value={settings.pur_base_price_3cm} onChange={v => onSettingChange('pur_base_price_3cm', v)} />
                        <PriceRow label="Hausse de prix / cm (jusqu'à 10cm)" value={settings.pur_step_price_up_to_10cm} onChange={v => onSettingChange('pur_step_price_up_to_10cm', v)} unit="€/cm" />
                        <PriceRow label="Prix / cm suppl. (au-delà 10cm)" value={settings.pur_extra_price_above_10cm} onChange={v => onSettingChange('pur_extra_price_above_10cm', v)} unit="€/cm" />
                        <PriceRow label="Rabais par palier de surface (100m²)" sublabel="Appliqué à >100, >200, >300 m²" value={settings.pur_surface_discount_step} onChange={v => onSettingChange('pur_surface_discount_step', v)} unit="€" />
                        <PriceRow label="Montant minimum d'exécution" sublabel="Plafond minimum pour petit chantier" value={settings.pur_minimum_execution_price} onChange={v => onSettingChange('pur_minimum_execution_price', v)} unit="€" />
                        
                        <SectionHeader label="Options Supplémentaires" />
                        <PriceRow label="Aspiration" value={settings.pur_opt_aspiration} onChange={v => onSettingChange('pur_opt_aspiration', v)} />
                        <PriceRow label="Niveller, travail au laser" value={settings.pur_opt_niveller} onChange={v => onSettingChange('pur_opt_niveller', v)} />
                        <PriceRow label="Ponçage de la mousse" sublabel="(obligatoire pour chauffage au sol)" value={settings.pur_opt_poncage} onChange={v => onSettingChange('pur_opt_poncage', v)} />
                        <PriceRow label="Protection au-dessus 1M" value={settings.pur_opt_protection} onChange={v => onSettingChange('pur_opt_protection', v)} />
                    </>
                )}

                {activeTab === 'eps' && (
                    <>
                        <SectionHeader label="Grille Volumétrique EPS" />
                        <div className="mt-2 text-[11px] text-slate-500 mb-4 leading-relaxed">
                            Les prix de l'isolation EPS sont calculés par <strong>mètre cube (m³)</strong> (Surface * Épaisseur). <br/>
                            Veuillez définir les paliers de volume. Laissez <strong>Prix Fixe</strong> vide pour appliquer le tarif par m³. Laissez <strong>Tarif / m³</strong> vide si c'est un montant d'exécution minimum.
                        </div>

                        <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700 overflow-hidden">
                            <div className="px-4 py-2 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between">
                                <span className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest">Paliers de Prix</span>
                            ...
                            <div className="p-3">
                                <div className="grid grid-cols-4 gap-2 pb-1 border-b border-slate-200 dark:border-slate-700 mb-2">
                                    ...
                                    <span className="text-[10px] font-bold text-slate-400 uppercase">Prix Fixe (€)</span>
                                    <span className="text-[10px] font-bold text-slate-400 uppercase">Tarif (€/m³)</span>
                                    <span />
                                </div>
...
with open(file_path, "w", encoding="utf-8") as f:
    f.write(content)
print("Updated PricingSettingsForm.jsx")
