...
export default function WorkOrderForm() {
    ...
    form.volumes.forEach(vol => {
        ...
        if (labelSafe.includes('sapa') || labelSafe.includes('chape')) {
            ...
            autoBase += (getPrice(form.prices?.base, surfaceForAuto <= 200 ? pricingSettings?.base_price_sqm : pricingSettings?.base_price_sqm_large, 12.5) * surfaceForAuto);
            const extraPrice = getPrice(form.prices?.extra, surfaceForAuto <= (pricingSettings?.extra_thickness_large_threshold_sqm ?? 200) ? pricingSettings?.extra_thickness_price_per_cm : pricingSettings?.extra_thickness_price_per_cm_large, 1.25);
            autoExtra += extraThickness * extraPrice * surfaceForAuto;
            autoFoil += vol.has_foil ? getPrice(form.prices?.foil, pricingSettings?.plastic_foil_price_sqm, 1.2) * surfaceForAuto : 0;
            autoMesh += vol.has_mesh ? getPrice(form.prices?.mesh, pricingSettings?.metal_mesh_price_sqm, 2.5) * surfaceForAuto : 0;
            ...
            autoFiber += vol.has_fiber ? fiberPrice * surfaceForAuto : 0;
        }
    });
            ...
            <Section icon={Calendar} title={t('work_order_form.planning_and_team', 'Planification & Équipe')} zIndex={50}>
                <div className="flex flex-col xl:flex-row gap-6 md:gap-8">
                    ...
                    <Field label={t('work_order_form.start_time', 'Heure de Début')}>
                        ...
                        <div className="mt-3">
                            <label className={`flex items-center gap-3 cursor-pointer p-2.5 rounded-xl border transition-colors ${form.send_notification ? 'bg-blue-50 border-blue-200 dark:bg-blue-900/20 dark:border-blue-800/50' : 'bg-slate-50 border-transparent hover:border-slate-200 dark:bg-slate-800/50 dark:hover:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800'}`}>
                                <input 
                                    type="checkbox" 
                                    checked={form.send_notification || false}
                                    onChange={e => set('send_notification', e.target.checked)}
                                    className="w-4 h-4 rounded text-blue-600 focus:ring-blue-500 border-slate-300 dark:border-slate-600 dark:bg-slate-700 cursor-pointer" 
                                />
                                <div className="flex flex-col">
                                    <span className={`text-sm font-semibold leading-tight ${form.send_notification ? 'text-blue-900 dark:text-blue-100' : 'text-slate-700 dark:text-slate-300'}`}>{t('planning.send_notification', 'Envoyer une notification au client')}</span>
                                    <span className={`text-[10px] leading-tight mt-0.5 ${form.send_notification ? 'text-blue-700 dark:text-blue-300' : 'text-slate-500 dark:text-slate-400'}`}>{t('planning.notification_desc', 'Email et WhatsApp avec les détails du rendez-vous')}</span>
                                </div>
                            </label>
                        </div>
                    </div>
