                    ...
                    <Section icon={CheckCircle2} title={t('work_order_detail.status_confirmations.title', "Confirmări Status")} contentClassName="!p-3">
                        <div className="flex flex-col sm:flex-row gap-4">
                            <div className="flex-1">
                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">{t('work_order_detail.general_details.team_leader_short', 'Șef Echipă')}</p>
                                {wo.team_leader_confirmed_at ? (
                                    <div className="flex flex-col gap-1">
                                        ...
                                        <span className="text-xs text-slate-600 dark:text-slate-400 font-semibold pl-4">{fmtFull(wo.team_leader_confirmed_at)}</span>
                                    </div>
                                ) : (
                                    <div className="flex items-center gap-1.5 h-8 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-dashed border-slate-200 dark:border-slate-700 px-2">
                                        <Circle className="w-3 h-3 text-slate-300 dark:text-slate-600 shrink-0" />
                                        <span className="text-[11px] font-semibold text-slate-400">{t('work_order_detail.status.awaiting_confirmation', 'În așteptare')}</span>
                                    </div>
                                )}
                                {wo.team_leader_confirmation_note && (
                                    <p className="text-[11px] text-slate-500 italic mt-1.5 bg-slate-50 dark:bg-slate-800 p-1.5 rounded-md border border-slate-100 dark:border-slate-700">{wo.team_leader_confirmation_note}</p>
                                )}
                            </div>
                            <div className="w-px bg-slate-100 dark:bg-slate-700 hidden sm:block"></div>
                            <div className="flex-1 border-t sm:border-t-0 border-slate-100 dark:border-slate-700 pt-3 sm:pt-0">
                                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">{t('work_order_detail.general_details.client_beneficiary', 'Client')}</p>
                                {wo.confirmed_at ? (
                                    <>
                                        <div className="flex items-center justify-between text-xs border-b border-slate-50 dark:border-slate-700/50 pb-1 mb-1">
                                            <span className="font-semibold text-emerald-600 truncate">{wo.confirmed_by_name}</span>
                                        </div>
                                        <div className="text-[11px] font-semibold text-slate-500 mb-1">{fmtFull(wo.confirmed_at)}</div>
                                        {hasSig && (
                                            <div className="flex justify-start">
                                                <div className="bg-white dark:bg-slate-900 rounded border border-emerald-200 px-1.5 py-0.5 inline-flex items-center justify-center">
                                                    <img src={wo.client_signature} alt="Semnătură" className="h-6 object-contain" />
                                                </div>
                                            </div>
                                        )}
                                    </>
                                ) : (
                                    <div className="flex items-center justify-center h-8 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-dashed border-slate-200 dark:border-slate-700">
                                        <p className="text-[11px] text-slate-400 font-medium">{t('work_order_detail.status.not_confirmed_by_client', 'Neconfirmat')}</p>
                                    </div>
                                )}
                            </div>
                        </div>
