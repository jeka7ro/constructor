...
export default function QuoteCalculator() {
    ...
    const handleSubmit = async (e) => {
        ...
        if (!validateEmail(formData.client_email)) {
            setError(t('invalid_email', 'Veuillez entrer une adresse e-mail valide.'));
            return;
        }
        ...
        try {
            ...
            if (res.data.token) {
                ...
                const webflowLang = formData.client_language === 'fr' ? '' : `/${formData.client_language}`;
                const redirectUrl = `https://davide-chape.webflow.io${webflowLang}/confirmation-contact?token=${res.data.token}&lang=${formData.client_language}`;
                        ...
                        {step === 2 && (
                            <div className="animate-in fade-in slide-in-from-right-4 duration-300 space-y-5 pb-48 sm:pb-60">
                                ...
                                <AddressAutocomplete
                                    ...
                                    onChange={(val) => {
                                        const countryStr = formData.client_language === 'fr' ? 'Belgique' : formData.client_language === 'nl' ? 'België' : formData.client_language === 'en' ? 'Belgium' : 'Belgia';
                                        const addr = val.includes(countryStr) ? val : `${val}, ${countryStr}`;
                                        setFormData(p => ({ ...p, site_address: addr }));
                                    }}
                                    onSelect={(addrObj) => setFormData({ ...formData, site_address: addrObj.address })}
                                    placeholder={t('calculator.address_placeholder', 'Adresse complète du chantier')}
                            ...
                            <div className="animate-in fade-in slide-in-from-right-4 duration-300 space-y-4">
                                ...
                                <div>
                                    ...
                                    <input type="email" required value={formData.client_email}
                                        ...
                                        onInvalid={e => e.target.setCustomValidity(t('clients.email_invalid', 'Veuillez entrer une adresse email valide (ex: contact@domaine.com)'))}
                                ...
                                <div>
                                    ...
                                    <input type="text" value={formData.client_address}
                                        onChange={(val) => {
                                            const countryStr = formData.client_language === 'fr' ? 'Belgique' : formData.client_language === 'nl' ? 'België' : formData.client_language === 'en' ? 'Belgium' : 'Belgia';
                                            const addr = val.includes(countryStr) ? val : `${val}, ${countryStr}`;
                                            setFormData(p => ({ ...p, client_address: addr }));
                                        }}
                                        className="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-3 py-2.5 text-base focus:bg-white focus:border-yellow-400 outline-none transition-all" />
                                </div>
